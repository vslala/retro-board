...
const LandingPage: React.FunctionComponent<Props> = (props) => {
    ...
    const getReferrerUrl = () => {
        if (location.state) {
            return (location.state as { referrer: string }).referrer
        }
        return "/"
    }
    ...
    const tryGoogleLogin = async () => {
        let isAuth = await state.firebase.isUserAuthenticated()
        if (!isAuth) {
            state.firebase.authenticateUser().then(() => {
                props.success()
                navigate(getReferrerUrl())
            })
        }
    ...
    const tryAnonymousLogin = async () => {
        if (!await state.firebase.isUserAuthenticated()) {
            state.firebase.authenticateAnonymousUser().then(() => {
